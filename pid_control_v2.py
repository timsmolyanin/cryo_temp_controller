...
class PIDControl(Thread):
    def __init__(self, mqtt_broker:str, mqtt_port:int, mqtt_user: str, mqtt_passw:str,
        ...
        self.__mqtt_topic_to_subscribe = mqtt_topics_pid
        self.__calculate_pid_limits = True
        self.__pid_min = 0.0
        self.__pid_max = 0.0
...
#<---New--->
# Новый метод. 
# Выбирает режим работы PID.
# Принимает: (self, ссылку на функцию).
# Возвращает: (ничего).                                      
    def set_run_mode(self):
        #self.run_mode = ссылка на функцию
        pass   
    
#<---Updated--->     
#Я не уверен, но представляю себе метод run вот так             
    def run(self):
        #Логгер
        logger.debug(f"Regulator {self.name} is started")
        
        #Выбираем режим управления
        self.set_run_mode(self, self.mode_auto)
    ...
    #<---Deprecated--->
    # Мы не должны ничего вычислять / фильтровать
    ...
    def enable_calculate_pid_limits(self, state: bool):
        self.__calculate_pid_limits = state
    ...
    #Не знаю, нам разве нужно изменять список топиков, на которые подписан PID?
    ...
    def set_control_loop_period(self, loop_period: float):
        ...
        if isinstance(loop_period, float):
            ...
            raise TypeError(f"Type for loop_period must be float, not {type(loop_period)}!")
    ...
    #Не уверен, мы разве должны что-то фильтровать?
    ...
    #Здесь лучше сделать словарь, как я первый раз делал с mqqt_bridge'ом 
    #Ключ : str, Значение : ссылка на метод
    #Например 
    #[
        #'PID Kp' : self.set_pid_kp_value(topic_value)
    #]
            ...
            match topic_name[2]:
                ...
                case "MeasureModule":
                    match topic_name[-1]:
                        case "CH1 Current":
                            self.mqtt_set_feedback(topic_val)
        elif self.name == "Temperature 1":
            match topic_name[2]:
                case "MeasureModuleSetpoints":
                    match topic_name[-1]:
                        case "CH1 Temperature Setpoint":
                            self.mqtt_set_setpoint(topic_val)
                        case "CH1 State":
                            self.set_control_value_state(bool(int(topic_val)))
                            self.set_pid_automode_value(bool(int(topic_val)))
    ...
    #<---Deprecated--->
    #Нам не нужно ничего вычислять / фильтровать 
